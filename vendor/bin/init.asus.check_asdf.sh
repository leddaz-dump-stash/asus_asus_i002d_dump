...
#logupload=`getprop persist.vendor.asus.autoupload.enable`
...
act_prop='persist.vendor.radio.check_asdf'
...
if [ ".$act_code" != "." ]; then
	...
	if [ ".$act_code" == ".2733" ]; then
		...
		input keyevent 82
			setprop persist.vendor.asus.uts "android.intent.action.MEDIA_MOUNTED --ez read-only false -d file:///storage/emulated/0/"
		...
		setprop vendor.debug.hotspot.result 200
