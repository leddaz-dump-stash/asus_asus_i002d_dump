...
#	mount -t vfat /dev/block/bootdevice/by-name/logfs /vendor/logfs
	...
	if [ ".$isBetaUser" == ".1" ]; then
		...
		if [ -f "/data/vendor/wifi/hostapd/hostapd_dual2g.conf" ]; then
			cp -r /data/vendor/wifi/hostapd/hostapd_dual2g.conf $GENERAL_LOG
			echo "cp -r /data/vendor/wifi/hostapd/hostapd_dual2g.conf $GENERAL_LOG"
		fi

		if [ -f "/data/vendor/wifi/hostapd/hostapd_dual5g.conf" ]; then
			cp -r /data/vendor/wifi/hostapd/hostapd_dual5g.conf $GENERAL_LOG
			echo "cp -r /data/vendor/wifi/hostapd/hostapd_dual5g.conf $GENERAL_LOG"
		fi

		if [ -f "/data/vendor/wifi/hostapd/hostapd_owe.conf" ]; then
			cp -r /data/vendor/wifi/hostapd/hostapd_owe.conf $GENERAL_LOG
			echo "cp -r /data/vendor/wifi/hostapd/hostapd_owe.conf $GENERAL_LOG"
		fi

		if [ -f "/data/vendor/wifi/hostapd/hostapd_legacy.conf" ]; then
			cp -r /data/vendor/wifi/hostapd/hostapd_legacy.conf $GENERAL_LOG
			echo "cp -r /data/vendor/wifi/hostapd/hostapd_legacy.conf $GENERAL_LOG"
		fi

		if [ -f "/data/vendor/wifi/hostapd/hostapd.accept" ]; then
			cp -r /data/vendor/wifi/hostapd/hostapd.accept $GENERAL_LOG
			echo "cp -r /data/vendor/wifi/hostapd/hostapd.accept $GENERAL_LOG"
		fi

		if [ -f "/data/vendor/wifi/hostapd/hostapd.deny" ]; then
			cp -r /data/vendor/wifi/hostapd/hostapd.deny $GENERAL_LOG
			echo "cp -r /data/vendor/wifi/hostapd/hostapd.deny $GENERAL_LOG"
		fi
...
export_tombstones_log() {
	...
	cp -r  /data/vendor/tombstones/SDX55M $ROOT_PATH/tombstones
	chmod -R 777 $ROOT_PATH/tombstones
	echo "/data/vendor/tombstones/SDX55M $ROOT_PATH/tombstones"
}
...
save_general_log() {
	...
	############################################################################################
	# save pdpolicy log
	cat /d/ipc_logging/usb_pd/log > $GENERAL_LOG/log_pdpolicy.txt
	echo "cat /d/ipc_logging/usb_pd/log > $GENERAL_LOG/log_pdpolicy.txt"
	############################################################################################
	cat /sys/kernel/dload/dload_mode > $GENERAL_LOG/dload_mode.txt
	cat /sys/module/msm_poweroff/parameters/download_mode > $GENERAL_LOG/download_mode.txt
	############################################################################################
...
#	mount -t vfat /dev/block/bootdevice/by-name/logfs /vendor/logfs
	...
	cat /d/regmap/spmi0-02/data > $GENERAL_LOG/charger_debug/pmic_reg.txt
	cat /d/regmap/5-0010/data > $GENERAL_LOG/charger_debug/smb1390_reg.txt
	cat /d/pmic-votable/*/status > $GENERAL_LOG/charger_debug/pmic_voter.txt
	cat /sys/class/power_supply/*/uevent > $GENERAL_LOG/charger_debug/uevent.txt
	cat /batinfo/.bh > $SAVE_LOG_PATH/charger_debug/bat_health.txt
	############################################################################################
